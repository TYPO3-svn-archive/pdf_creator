 ...
 * @version 1.10
 */
#define('WKHTML_BIN',t3lib_extMgm::extPath('pdf_creator','bin/wkhtmltopdf-amd64'));
define('WKHTML_BIN',t3lib_extMgm::extPath('pdf_creator','bin/wkhtmltopdf'));
    
class pc_wkhtmltopdf
{
    /**
    ...
    /**
     * returns value is enabled javascript or not
     *
     * @author arieedzig <arieedzig@yahoo.com>
     * @return boolean
     */
     ...
     * @param string $header
     * @return Wkthmltopdf
     * @author arief <arief.rakhman@hs-offenburg.de>
     */
    ...
    {
        $mtype = $this->_pipeExec("uname -m");
        $binm = preg_match('(x86_64)', $mtype['stdout']) ? "-amd64" : "-i386" ;
        $command = $this->_bin . $binm;
    ...
    protected function _render()
    {
        ...
        $content = $this->_pipeExec(str_replace('%input%', $input, $this->_getCommand()));
        unlink($input);
    #$content = str_replace('%input%', $input, $this->_getCommand());
    #throw new Exception("WKHTMLTOPDF ".$content);

        if (strpos(mb_strtolower($content['stderr']), 'error'))
                throw new Exception("This page cannot be converted to pdf version");
